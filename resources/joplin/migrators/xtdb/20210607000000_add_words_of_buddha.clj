(ns joplin.migrators.xtdb.20210607000000-add-words-of-buddha
...
(defn up [db]
  (let [node (d/get-node (:conf db))]
    (schema/add-type node :words-of-buddha [:words-of-buddha/original-words ;; from *.pariyatti.org - a long string
                                            ...
                                            :words-of-buddha/original-audio-url      ;; to *.pariyatti.org - mp3
    ...
    (schema/add-schema node :words-of-buddha/original-audio-url      :db.type/uri)
...
(defn down [db]
  (let [node (d/get-node (:conf db))]
    ...
    (schema/remove-schema node :words-of-buddha/original-audio-url)
