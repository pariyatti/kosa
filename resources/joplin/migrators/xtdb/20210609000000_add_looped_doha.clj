...
(defn up [db]
  (let [node (d/get-node (:conf db))]
    (schema/add-type node :looped-doha [:looped-doha/index
                                        ...
                                        :looped-doha/original-audio-url ;; to *.pariyatti.org - mp3
                                        :looped-doha/doha
    ...
    (schema/add-schema node :looped-doha/original-audio-url  :db.type/uri)
...
(defn down [db]
  (let [node (d/get-node (:conf db))]
    ...
    (schema/remove-schema node :looped-doha/original-audio-url)
