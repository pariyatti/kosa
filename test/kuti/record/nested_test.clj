...
(deftest collapse-one
  (testing "collapses a named field"
    (let [entity {:zig-attachment
                  {:xt/id id1 :attm/filename "this-zig.txt"}
                  :zag-attachment
                  {:xt/id id2 :attm/filename "this-zag.txt"}}
...
(deftest collapse-all
  (let [doc {:name "List of plants"
             :tree-plant {:name "Pine" :xt/id id1}
             :shrub-plant {:name "Heather" :xt/id id2}}
...
(deftest expand-one
  (let [tree {:xt/id id1
              :tree/name "Birch"}
        shrub {:xt/id id2 :shrub/name "Sage"}
        doc {:xt/id id3
